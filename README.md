...
本系统可依据《计算机软件著作权登记办法》申请软件著作权登记。申请材料与工具如下：

| 材料 | 来源 | 说明 |
|---|---|---|
| 软件全称 | antipoleBGP 出口调度管理系统 | 申请表中填写，全部材料保持一致 |
| 软件简称 | antipoleBGP | — |
| 版本号 | V1.1 | 申请表、源程序页眉、文档页眉一致 |
| 源程序鉴别材料 | `tools/gen_copyright.py` 生成至 `copyright/` | 前 30 页 + 后 30 页，每页 50 行，页眉含软件名称/版本号/页码 |
| 文档鉴别材料 | `docs/用户操作手册.md` | 转 Word 后补充 3~5 张实际运行截图 |
| 申请表 | 中国版权保护中心官网 | 按系统信息如实填写 |

生成源程序鉴别材料：

```bash
python tools/gen_copyright.py
```

提交前注意事项：

1. 所有材料中的软件名称与版本号必须与申请表完全一致；
2. 源程序每页不少于 50 行、无大面积空白行、无与申请表冲突的注释；
3. 文档每页不少于 30 行，须包含实际运行截图（命令输出或 API 返回）；
4. 提交 Word 格式，A4 纸张，页眉标注"antipoleBGP 出口调度管理系统 V1.1"。
...
| 版本 | 说明 |
|---|---|
| V1.1 | 重构安装脚本（发行版/架构自适应、依赖按需安装、二进制 SHA256 校验）；地址池支持 /32~/64 任意前缀；移除 Web 管理界面，管理操作全面迁移至 CLI；新增只读令牌、Prometheus 指标端点、WARP 多 endpoint 容错、证书到期告警；修复 tc 限速规则误删、配额超限窗口、孤儿进程、CLI 与服务并发冲突等问题；统一文件权限与安全基线；配套软件著作权登记材料（源程序鉴别材料生成工具、用户操作手册） | |
...
antipoleBGP 出口调度管理系统版权所有。未经授权不得用于商业分发；部署使用请遵守所在国家/地区法律法规。
